...
# Covers /p/, /reel/, /reels/, /tv/, /stories/<username>/, /share/
_INSTAGRAM_URL_REGEX = (
    r"(https?://(www\.)?instagram\.com/"
    r"(p|reel|reels|tv|stories/[^/?#&]+|share)/[^/?#&]+)"
)
...
@router.message(
    F.text.regexp(_INSTAGRAM_URL_REGEX, mode="search")
)
...
async def process_instagram(message: types.Message, direct_url: Optional[str] = None):
    ...
    try:
        ...
        if direct_url:
            ...
            url_match = re.search(_INSTAGRAM_URL_REGEX, text)
            if not url_match:
                return
...
@router.inline_query(
    F.query.regexp(_INSTAGRAM_URL_REGEX, mode="search")
)
